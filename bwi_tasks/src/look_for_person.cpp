...
bool goToInitialState(Client &client) {
  ...
  bwi_kr_execution::AspRule initial_rule;
  ...
  initial_rule.body.push_back(front_of_lab_door);
  
  go_to_initial_state.aspGoal.push_back(initial_rule);
...
void resetMemory() {
  ...
  remove_copy_if(query.response.answer.fluents.begin(), query.response.answer.fluents.end(), back_inserter(fluents_to_keep), TaskFluent());
