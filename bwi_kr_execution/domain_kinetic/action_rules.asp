...
% action navigate_to
is_located(self, T, n) :- navigate_to(T, n).
-is_located(self, F, n) :- navigate_to(T, n), is_located(self, F, n - 1), T != F.
...
:- find_person(P, n), 0{is_located(self, L, n - 1) : can_be_located(P, L); is_located(self, L, n - 1) : is_located(P, L)}0.
...
:- pick_up(O, L, n), not is_located(self, L, n - 1).
...
:- put_down(O, L, n), not is_located(self, L, n - 1).
...
:- perceive_surface(L, n), not is_located(self, L, n - 1).
...
%%inertial rules
is_located(O, L, n) :- is_located(O, L, n - 1), not -is_located(O, L, n).
...
%skip findPerson for operator
is_facing(self, P, n) :- is_located(self, L, n), is_located(P, L, n), has_concept(P, "operator").

% allow passing concept name in goal query
is_located_concept(O_concept, L_concept, n) :- is_located(O_id, L_id, n), has_concept(O_id, O_concept), has_concept(L_id, L_concept).
...
is_located(O, L, 0) :- is_located(O, L).
...
is_located_concept(O_concept, L_concept, 0) :- is_located(O_id, L_id), has_concept(O_id, O_concept), has_concept(L_id, L_concept).
...
%skip findPerson for operator
is_facing(self, P, 0) :- is_located(self, L, 0), is_located(P, L, 0), has_concept(P, "operator").
