...
std::list<actasp::AnswerSet> Clingo4_5::monitorQuery(const std::vector<actasp::AspRule>& goalRules,
  ...
  list<actasp::AnswerSet> result = genericQuery(monitorQuery,plan.fluents.size(),plan.fluents.size(),"monitorQuery",1);

  result.remove_if(MaxTimeStepLessThan4_5(plan.fluents.size()));
...
std::list<actasp::AnswerSet> Clingo4_5::filteringQuery(const AnswerSet& currentState, const AnswerSet& plan,const std::vector<actasp::AspRule>& goals) {
  ...
  return  genericQuery(total.str(),plan.fluents.size(),plan.fluents.size(),"filterState",0, false);
