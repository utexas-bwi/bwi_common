...
#include "actasp_ros/ExecutePlanAction.h"
...
#include <actasp_ros/PlanExecutorNode.h>
...
PlanExecutorNode::PlanExecutorNode(const string &domain_directory, map<string, ActionFactory> action_map,
  ...
  planningReasoner = unique_ptr<actasp::Solver>(actasp::Clingo::getSolver(domain_directory));
  auto diagnosticsPath = boost::filesystem::path(domain_directory) / "diagnostics";
  if (boost::filesystem::is_directory(diagnosticsPath)) {
    auto diagnosticReasoner = std::unique_ptr<actasp::Solver>(actasp::Clingo::getSolver(diagnosticsPath.string()));
    ros_observer = std::unique_ptr<RosActionServerInterfaceObserver>(
        new ExplainingRosActionServerInterfaceObserver(server, std::move(diagnosticReasoner)));
  } else {
  ...
  }
  {
    ...
    auto replanner = new ReplanningPlanExecutor(*dynamic_cast<actasp::AspKR *>(planningReasoner.get()),
                                                *dynamic_cast<actasp::Planner *>(planningReasoner.get()), action_map,
                                                resourceManager);
...
void PlanExecutorNode::executePlan(const actasp_ros::ExecutePlanGoalConstPtr &plan) {
  actasp_ros::ExecutePlanResult result;
  vector<actasp::AspRule> goalRules;
  ...
  while (!executor->goalReached() && !executor->failed() && ros::ok() && server.isActive()) {
    if (!server.isPreemptRequested()) {
      ...
      if (server.isNewGoalAvailable()) {
        goalRules.clear();
        const actasp_ros::ExecutePlanGoalConstPtr &newGoal = server.acceptNewGoal();
