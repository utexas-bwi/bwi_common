...
int warningDistance = 3;

class Visit_Door_safely {
public:
  Visit_Door_safely(ros::NodeHandle* nh);
  void run(bool REVERSE);
  void callback(const nav_msgs::Odometry::ConstPtr &odom, const nav_msgs::Odometry::ConstPtr &other_odom);
  vector<geometry_msgs::Point> getInteractionPoints(const geometry_msgs::Point &start_pt, const geometry_msgs::Point &goal_pt);
  geometry_msgs::Point getInteractionPoint(const nav_msgs::Odometry::ConstPtr &my_odom, const nav_msgs::Odometry::ConstPtr &other_odom);
...
geometry_msgs::Point Visit_Door_safely::getInteractionPoint(const nav_msgs::Odometry::ConstPtr &my_odom, const nav_msgs::Odometry::ConstPtr &other_odom){
  vector<geometry_msgs::Point> interaction_points = getInteractionPoints(my_odom->pose.pose.position, other_odom->pose.pose.position);
...
void Visit_Door_safely::callback(const nav_msgs::Odometry::ConstPtr &my_odom, const nav_msgs::Odometry::ConstPtr &other_odom){
// Make a code without parked function first.
  ...
  geometry_msgs::Point interaction_point = getInteractionPoint(my_odom, other_odom);

  if(DEBUG){
    //ROS_INFO_STREAM("interaction point : "<<interaction_point.x<<" "<<interaction_point.y);
  }

  bool safe = safeInteraction(interaction_point, my_odom->pose.pose.position, other_odom->pose.pose.position);
/*  ROS_INFO_STREAM(interaction_point << "\n" << my_odom->pose.pose.position << "\n" << other_odom->pose.pose.position);
...
ROS_INFO_STREAM("Distance : " << distance_history.size());
  if(parked){
    ...
    string nearSafeZone = getNearestParkingZone(interaction_point, my_odom->pose.pose.position, other_odom->pose.pose.position);

    if(nearSafeZone.compare("empty") == 0){
      nearSafeZone = getNearestParkingZone(my_odom->pose.pose.position, my_odom->pose.pose.position, other_odom->pose.pose.position);
    }
    //ROS_INFO_STREAM("Roberto going to " + nearSafeZone);
    if(nearSafeZone.compare("empty") && DEBUG){
      myfile<<"my position:\n";
      myfile<<my_odom->pose.pose.position << "\t" << other_odom->pose.pose.position;
    }
...
void Visit_Door_safely::run(bool REVERSE){
  ...
  message_filters::Subscriber<nav_msgs::Odometry> my_odom(*nh_, "/roberto/odom", 1);
  message_filters::Subscriber<nav_msgs::Odometry> other_odom(*nh_, "/marvin/odom", 1);
  message_filters::TimeSynchronizer<nav_msgs::Odometry, nav_msgs::Odometry> sync(my_odom, other_odom, 10);
  sync.registerCallback(boost::bind(&Visit_Door_safely::callback, this, _1, _2));
  ros::spin();
