...
MRH_Experiment::MRH_Experiment(ros::NodeHandle* nh):marvin_ac("marvin/move_base"),roberto_ac("roberto/move_base")
{
  ...
  marvin_localizer = nh_->advertise<geometry_msgs::PoseWithCovarianceStamped>("/marvin/initialpose",1,true);
	...
	this->subscribeMarvinVel_   = nh_->subscribe("/roberto/cmd_vel", 1, &MRH_Experiment::subscribeMarvinCmdvel, this);
...
bool MRH_Experiment::checkCollision(){
	...
	if(this->robertoPlan_.size() && this->marvinPlan_.size()){
		if(dist(this->robertoPlan_[0].position, this->marvinPlan_[0].position) < 0.65){
			return true;
		}
	}
	return false;
...
vector<double> MRH_Experiment::run_raw_experiment(int timeout, ofstream& recRoberto, ofstream& recMarvin, int delay=0.0){
	...
	endtime.push_back(this->marvinX_); // Marvin X
	endtime.push_back(this->marvinY_); // Marvin Y
	...
	*/
	...
	while(ros::ok()){
		...
		if(this->checkCollision()){
			endtime[0] = 1000.0 + (ros::Time::now() - this->start_time).toSec();
			return endtime;
		}
...
int main(int argc, char** argv){
  ...
  // Set initial environment for expermients
	eh.setSpawnPose("marvin", 2.8, 18.1, 0.0);
  eh.setInitPose("marvin", 12.8, 17.45, PI/180.*0.); // Test: -PI/12 0 PI/12  150 each

	eh.setSpawnPose("roberto", 26.8, 18.1, PI);
  eh.setInitPose("roberto", 18.8, 18.1, PI);
	...
	for(int i=atoi(argv[2]); i<atoi(argv[3]); i++){
		...
		if(i%num_exp == 0 && new_file == true){
			...
			eh.setInitPose("marvin", 12.8, 17.45, PI/180. * angle);
			...
			outfile << "reset_time,roberto_time,marvin_X,marvin_Y,roberto_orientation,marvin_orientation,roberto_impact_degree"<<endl;
