...
*/
  //eh.showInitPose();
	...
	double reset_time, xdiff,ydiff;
	for(int i=0; i<atoi(argv[2]); i++){
		// -4 on x
		xdiff = xrand(e2);
		ydiff = yrand(e2);
		eh.setSpawnPose("marvin", 4.8 + xdiff, 18.1 + ydiff, PI);
		eh.setInitPose("marvin", 8.5 + xdiff, 18.1 + ydiff, 0.); // Test: -PI/12 0 PI/12  150 each
		eh.setGoalPose("marvin", 26.5 + xdiff, 18.1 + ydiff, 0.);
		eh.setStandbyPose("marvin", 28.45, 18.15, 0.0);
		eh.setSpawnPose("roberto", 22.8 + xdiff, 18.1 + ydiff, 0.0);
		eh.setInitPose("roberto", 18.5 + xdiff, 18.1 + ydiff, PI);
		eh.setGoalPose("roberto", 0.5 + xdiff, 18.1 + ydiff, PI);
